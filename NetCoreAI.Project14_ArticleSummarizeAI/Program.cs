...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        if (!string.IsNullOrEmpty(input)) // Eğer kullanıcı bir metin girdiyse
        {
            Console.WriteLine();
            Console.WriteLine("Girmiş olduğunuz metin AI tarafından özetleniyor...");
            Console.WriteLine();

            string shortSummary = await SummarizeText(input, "short"); // Kısa özet
            string mediumSummary = await SummarizeText(input, "medium"); // Orta uzunlukta özet
            string detailedSummary = await SummarizeText(input, "detailed"); // Detaylı özet
