...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        if (!string.IsNullOrEmpty(input))
        {
            ...
            Console.WriteLine();
            string sentiment = await AnalyzeSentiment(input); // Duygu analizi fonksiyonunu çağır

            Console.WriteLine($"Sonuç: {sentiment}");
        }


        static async Task<string> AnalyzeSentiment(string text) // Duygu analizi fonksiyonu
        {
